...
// import { LayoutDashboard, Book, User } from 'lucide-react'; // Removed unused icons
...
//           <Link href="/customer/dashboard" className="flex items-center p-2 text-gray-700 bg-gray-200 rounded-lg">
//             <LayoutDashboard className="mr-3"/>Dashboard
//           </Link>
//           <Link href="#" className="flex items-center p-2 text-gray-600 hover:bg-gray-200 rounded-lg">
//             <Book className="mr-3"/>Bookings
//           </Link>
//           {/* Profile Link Added */}
//           <Link href="/customer/profile" className="flex items-center p-2 text-gray-600 hover:bg-gray-200 rounded-lg">
          ...
          <div className="space-y-4">
            {bookings.length > 0 ? bookings.map(booking => (
              <div key={booking._id} className="p-4 border rounded-lg flex justify-between items-center">
                ...
                <span className={`px-3 py-1 text-sm rounded-full ${
                    booking.status === 'Completed' ? 'bg-green-200 text-green-800' : 
                    booking.status === 'Pending' || booking.status === 'Assigned' ? 'bg-yellow-200 text-yellow-800' :
                    'bg-blue-200 text-blue-800'
                }`}>{booking.status}</span>
              </div>
            )) : <p>You have no upcoming services.</p>}
