        ...
        const fetchUserData = async () => {
            try {
                ...
                setFormData({ 
                    ...
                    phone: data.data.profile?.phone || '' // Use ?. to prevent error if profile is undefined
                });
                // --- FIX END ---
            } catch (error) {
                toast.error("Could not fetch profile data.");
                // router.push('/login'); // Temporarily disable redirect for debugging
            }
        };
        if (token) {
            fetchUserData();
        ...
        <div className="flex min-h-screen bg-gray-100">
            ...
            <aside className="w-64 bg-white shadow-md p-4">
                ...
                <nav className="space-y-2">
                    <Link href="/customer/dashboard" className="flex items-center p-2 text-gray-600 hover:bg-gray-200 rounded-lg"><LayoutDashboard className="mr-3"/>Dashboard</Link>
                    <Link href="#" className="flex items-center p-2 text-gray-600 hover:bg-gray-200 rounded-lg"><Book className="mr-3"/>Bookings</Link>
                    <Link href="/customer/profile" className="flex items-center p-2 text-gray-700 bg-gray-200 rounded-lg"><User className="mr-3"/>Profile</Link>
                </nav>
